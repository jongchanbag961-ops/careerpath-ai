...
if submitted:
    ...
    st.markdown(
        '<div class="section-subtitle">응답 결과를 바탕으로 4개 진로경로 가능성을 산출했습니다.</div>',
        unsafe_allow_html=True,
    )
    ...
    if school_info_applied:
        ...
        if str(school_code_input).strip() == "":
            school_msg = "학교코드를 입력하지 않아 학생 응답 정보와 선택한 지역 정보를 중심으로 진단했습니다."
        else:
            school_msg = "입력한 학교코드를 찾을 수 없어 학교환경 정보는 반영하지 않고 기본 진단을 진행했습니다."
    ...
    st.markdown(
        """
        <div class="neutral-box">
        아직 진단 결과가 생성되지 않았습니다. 위의 문항을 입력한 뒤 <b>진단 결과 보기</b> 버튼을 눌러주세요.
        </div>
        """,
        unsafe_allow_html=True,
    )
