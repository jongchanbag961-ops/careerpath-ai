...
if submitted:
    ...
    if school_info_applied:
        ...
        try:
            school_location_int = int(school_location) if school_location is not None and pd.notna(school_location) else None
            selected_location_int = int(selected_location)
        except Exception:
            school_location_int = None
            selected_location_int = int(selected_location)

        if school_location_int is not None and school_location_int != selected_location_int:
            school_region_name = location_name_by_code.get(school_location_int, f"코드 {school_location_int}")
            st.markdown(
                f"""
                <div class="warning-box">
                ⚠️ 입력한 학교코드의 지역(<b>{school_region_name}</b>)과 선택한 지역(<b>{selected_region_name}</b>)이 다릅니다.<br>
                지역 변수는 선택한 지역 기준으로, 학교환경 변수는 입력한 학교코드 기준으로 반영했습니다.
                </div>
                """,
                unsafe_allow_html=True,
            )
