    ...
    if school_info_applied:
        ...
        if school_location is not None and school_location != selected_location:
            ...
            st.markdown(
                f"""
                <div class="neutral-box">
                입력한 학교코드의 지역({school_region_name})과 선택한 지역({selected_region_name})이 다릅니다. 
    ...
    with rec_col2:
        ...
        if school_info_applied:
            ...
            if school_recommendations:
                ...
                st.markdown("- 학교환경 정보가 반영되었지만, 세부 수준을 분류할 수 없어 기본 추천 방향을 우선 참고하세요.")
        else:
            st.markdown("- 학교코드를 입력하면 학교 진로환경 정보를 반영한 추가 제안을 확인할 수 있습니다.")
            st.markdown("- 학교코드를 모르는 경우 담당 교사 또는 상담교사에게 문의해보세요.")
